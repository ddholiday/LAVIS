...
class Blip2ITM(Blip2Qformer):
    ...
    def compute_gradcam(model, visual_input, text_input, tokenized_text, block_num=8):
        model.Qformer.bert.encoder.layer[block_num].crossattention.self.save_attention = True
        ...
        with torch.no_grad():
            ...
            grads = model.Qformer.bert.encoder.layer[block_num].crossattention.self.get_attn_gradients()
            cams = model.Qformer.bert.encoder.layer[block_num].crossattention.self.get_attention_map()
            # assume using vit with 576 num image patch
            cams = cams[:, :, :, 1:].reshape(visual_input.size(0), 12, -1, 26, 26) * mask
            grads = (
                grads[:, :, :, 1:].clamp(0).reshape(visual_input.size(0), 12, -1, 26, 26) * mask
            )
